...
### 横断原則（全要件共通・必須）: スコアを語る文には必ず「具体例」を添える

R1–R5 のどの要件でも、**抽象的な一般論だけで終わらせず、読者（数か月後の自分・LLM）が手で追える具体例を最低 1 つ添えること**。R3 の「具体的なデータ特徴で書く」を全要件に拡張したもの。最低限つけるべき具体例:

- **指標の挙動**: 「予測 `[C, A, B]` で正解 C なら 1.0、`[A, C, B]` なら 0.5、圏外なら 0.0」のような **実値の計算例**（数式の説明だけで終えない）
- **設問特性 (R3)**: 「数値設問は落ちる」で終えず、`"When was radium discovered?"` のような **実際の設問文イメージ**と、できれば distractor（誤答選択肢）の例まで（例: 正解 "covalent bond" vs distractor "polar covalent bond"）
- **誤差・ギャップ (R1/R5)**: 「val=40 は ±0.07 ぶれる」で終えず、「32/40 正解が別の 40 行なら 29〜35 行に揺れ、val は 0.72〜0.88 に散らばる」と **行数・数値で例示**
- **スコア比較 (R2/R4)**: 「Public < Private」で終えず、「m2 は Public 0.682 < Private 0.714」と **実数を並べて**示す

→ 数字・設問例・計算例のどれも無い「一般論だけの考察」は不可（下の NG 例参照）。
...
- ❌ 抽象的な特徴語（「long-tail 設問」「編集距離が小さい」「楽観バイアス」等）だけで、**実際の設問文・選択肢・数値の具体例**を 1 つも添えていない（横断原則違反）

### 既存文書への遡及適用

R1-R5 規約は **Cycle 01 結果と考察 (`knowledge/01/cycle01_2_results_and_analysis.md`) を含む既存の浅い考察文書** に遡及適用する（この文書は適用済み）。次の Cycle 開始前に更新済みかをチェック。
